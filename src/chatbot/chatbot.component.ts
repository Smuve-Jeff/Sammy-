...
export class ChatbotComponent implements OnInit, OnDestroy {
  ...
  isAiAvailable = computed(() => (this.aiService as AiService).isAiAvailable());
  ...
  async sendStandardMessage(message: string): Promise<void> {
    try {
      const response = await (this.aiService as AiService).chatInstance?.sendMessage({ message });
      if (response) {
  ...
  async sendGoogleSearchQuery(query: string): Promise<void> {
    try {
      const response = await (this.aiService as AiService).genAI?.models.generateContent({
  ...
  async transcribeAudio(base64Audio: string, mimeType: string): Promise<void> {
    this.isLoading.set(true);
    try {
      const transcription = await (this.aiService as AiService).transcribeAudio(base64Audio, mimeType);
  ...
  private giveContextualAdvice(mode: MainViewMode) {
    let advice = '';
    const lastTheme = (this.userContext as any).lastUsedTheme();
    const lastImage = (this.userContext as any).lastGeneratedImageUrl();
    const profile = (this.userProfileService as any).profile();
  ...
  private buildContextualPrompt(message: string): string {
    const mode = this.mainViewMode();
    const theme = (this.userContext as any).lastUsedTheme();
    const lastImage = (this.userContext as any).lastGeneratedImageUrl();
    const profile = (this.userProfileService as any).profile();
